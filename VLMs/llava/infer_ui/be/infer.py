...
def build_model_and_tokenizer(model_args, data_args, training_args, attn_implementation=None):
    ...
    # Load quantization config nếu có
    bnb_model_from_pretrained_args = {}
    if training_args.bits in [4, 8]:
        bnb_model_from_pretrained_args.update(dict(
            device_map={"": training_args.device},
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(
                ...
                cache_dir=training_args.cache_dir,
                attn_implementation=attn_implementation,
                torch_dtype=compute_dtype if training_args.fp16 or training_args.bf16 else None,
                **bnb_model_from_pretrained_args
            )
    ...
    # Load LoRA nếu có
    if training_args.lora_enable:
        print(f"Loading LoRA adapter from {model_args.lora_path}")
        from peft import PeftModel 
...
def test_one_case(
    ...
    # Prepare question
    qs = preprocess_question(question_text)
    conv = conv_templates[conv_mode].copy()
    conv.append_message(conv.roles[0], qs)
    conv.append_message(conv.roles[1], None)
    prompt = conv.get_prompt()

    # Tokenize prompt with image placeholder token
    input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(device)

    pet_tensor = process_image(pet_image)
    ct_tensor = process_image(ct_image, is_pet=False)

    image_inputs = {
        'PET': pet_tensor.unsqueeze(0).to(dtype=dtype, device=device),
        'CT': ct_tensor.unsqueeze(0).to(dtype=dtype, device=device)
    }
