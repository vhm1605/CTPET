...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        if 'PET' in images and 'CT' in images:
            ...
            with open('/home/nvidia-lab/ai4life/CTPET/labels_filtered.json', 'r') as f:
            ...
            for seg, path in zip(ct_segs, paths):
                ...
                # print(seg.dtype)
                # print(valid_ids.dtype)
                keep_mask = torch.isin(seg.float(), valid_ids.float())
            ...
            # print(f"Sample {i}: #PET patches={len(pet_patch_tensor[i])}, #CT patches={len(ct_patch_tensor[i])}")

                # ==================== PET ====================
                pet_embs_list = []
                for idx, p in enumerate(pet_patch_tensor[i]):
                    if p is None:
                        continue
                    ...
                    # (D) hoặc (C,H,W) -> thêm batch + token dim
                    # print(p.shape)
                    emb = self.get_model().mm_projector(
                        p.unsqueeze(0),   # (1, 1, ...)
                        mode="focal_pet"
                    )
                    ...
                    pet_embs_list.append(emb)

                if len(pet_embs_list) > 0:
                    pet_embs_i = torch.stack(pet_embs_list, dim=0)   # (N_pet, D_out)
                else:
                    pet_embs_i = torch.empty(0, device=global_feat.device)


                # ==================== CT ====================
                ct_embs_list = []
                for idx, p in enumerate(ct_patch_tensor[i]):
                    if p is None:
                        continue
                    ...
                    ct_embs_list.append(emb)

                if len(ct_embs_list) > 0:
                    ct_embs_i = torch.stack(ct_embs_list, dim=0)
                else:
                    ct_embs_i = torch.empty(0, device=global_feat.device)


                # print("PET embs:", pet_embs_i.shape)
                # print("CT embs:", ct_embs_i.shape)
                ...
                common_labels = sorted(
                    set(pet_label_to_emb.keys()) & set(ct_label_to_emb.keys())
                )

                # print(
                #     "common:",
                #     len(common_labels),
                #     "pet:", len(pet_label_to_emb),
                #     "ct:", len(ct_label_to_emb)
                # )
                ...
                for lbl_id in common_labels:
                    ...
                    pet_emb = pet_label_to_emb[lbl_id].unsqueeze(0)  # (1, D)
                    ct_emb  = ct_label_to_emb[lbl_id].unsqueeze(0)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if tokenizer is not None:
            ...
            newline_embed = embed_tokens(newline_ids[0])

            # global text
            global_ids = tokenizer(
                "Global context:",
                add_special_tokens=False,
                return_tensors="pt"
            ).input_ids.to(device)
            global_embed = embed_tokens(global_ids[0])
        ...
        for b in range(B):
            ...
            # ===== GLOBAL =====
            if tokenizer is not None:
                cur_blocks.append(global_embed)

            cur_blocks.append(global_feat[b].to(device))

            if tokenizer is not None:
                cur_blocks.append(newline_embed)
            ...
            if is_train:
                ...
                min_keep = max(1, int(0.5 * num_regions))  
                keep_n = random.randint(min_keep, num_regions)

                region_indices = region_indices[:keep_n]
            ...
            for k in region_indices:
                ...
                # ===== REGION LABEL (CACHE) =====
                if tokenizer is not None:
                    if organ_name not in text_cache:
                        text = f"{organ_name}:"
                        ids = tokenizer(
                            text,
                            add_special_tokens=False,
                        ...
                        text_cache[organ_name] = embed_tokens(ids[0])

                    cur_blocks.append(text_cache[organ_name])

                # ===== PET =====
                cur_blocks.append(
                    pet_region_embeds_per_sample[b][k].squeeze(0).to(device)
                )

                # ===== CT =====
                cur_blocks.append(
                    ct_region_embeds_per_sample[b][k].squeeze(0).to(device)
                )
        ...
        # for i, emb in enumerate(new_input_embeds):
        #     print(f"[After MM] Sample {i} length:", emb.shape[0])
