    ...
    if model_args.vision_tower is not None:
        ...
        for i in tqdm(range(0, len(questions), batch_size)):
            ...
            for line in batch_questions:
                idx = line["id"]
                batch_question_ids.append(idx)

                # Handle image loading based on data type
                if data_args.type == 'PET/CT':
                    ...
                    image_size = pet_image.size
                else:
                    image_file = line["image"]
                    image = np.load(os.path.join(data_args.image_folder, image_file))
                    is_pet = False if data_args.type == 'CT' else True
                ...
                # Extract and preprocess question
                for convo in line["conversations"]:
                    if convo["from"] == "human":
                        qs = preprocess_question(convo["value"])
                        break
                cur_prompt = qs.replace("<image>", "").strip()  # Remove <image> placeholder
            ...
            batch_input_ids = batch_input_ids.to(training_args.device)

            # Prepare batched images
            if data_args.type == 'PET/CT':
                if not batch_images:
                    raise ValueError("batch_images is empty for PET/CT data")
                batch_pet_images = torch.cat([images['PET'] for images in batch_images], dim=0)
                batch_ct_images = torch.cat([images['CT'] for images in batch_images], dim=0)
                batch_images_dict = {'PET': batch_pet_images, 'CT': batch_ct_images}
            else:
                if not batch_images:
                    raise ValueError("batch_images is empty for non-PET/CT data")
                batch_data_images = torch.cat([images['data'] for images in batch_images], dim=0)
                batch_images_dict = {'data': batch_data_images}
            ...
            # Decode outputs and clean up
            outputs = tokenizer.batch_decode(output_ids, skip_special_tokens=True)
            outputs = [re.sub(r'[^\x00-\x7F\u0080-\uFFFF]+', '', output).strip() for output in outputs]  # Remove non-printable characters

            # Print prompts and answers for 10% of the batch randomly
            ...
            for idx, prompt, output in zip(batch_question_ids, batch_prompts, outputs):
